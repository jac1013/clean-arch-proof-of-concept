...
class Controller {
    ...
    Controller(String[] CLIArguments, AirplaneFactory airplaneFactory, FlightFactory flightFactory, SeatFactory seatFactory, CityFactory cityFactory) {
    ...
    private Optional<Reservation> constructReservation(PresenterRequest request) {
        Optional<PresenterResponse> optionalResponse = new PresenterDataTransformer(request, cityFactory).transform();
        if(optionalResponse.isPresent()) {
            ...
            return Optional.of(new OnlineReservator().bookFlight(flight, seat, response.getPassenger(), new ReservationFactoryImpl()));
        }
